...
import itertools
# import threading
# threading.stack_size(2**2d)
# import sys
# sys.setrecursionlimit(10**d)
...
for _ in range(inin()):
    ...
    a = lin()
    namta = []
    for i in range(1, math.ceil(max(a)/d)+1):
        r = i*d
        namta.append(str(r))
    print(namta)
    ...
    for i in range(q):
        ...
        if str(d) in str(a[i]):
            ...
            for j in range(1, len(namta)):
                if str(a[i])[-1] == namta[j][-1] and (j + 1) * d <= a[i]:
                    flag = "YES"
                    break
        
        print(str(a[i]), namta[j], (j + 1) * d)
        print(flag)
